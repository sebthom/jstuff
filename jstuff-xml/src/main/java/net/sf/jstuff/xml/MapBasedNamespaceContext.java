...
public class MapBasedNamespaceContext implements NamespaceContext
{
	...
	public void bindNamespace(final String namespaceURI, final String prefix)
	{
		...
		if (prefix.equals(XMLConstants.DEFAULT_NS_PREFIX)) defaultNamespaceURI = namespaceURI;

		if (!namespaceURIsByPrefix.containsKey(prefix)) namespaceURIsByPrefix.put(prefix, new ArrayList<String>(2));
		namespaceURIsByPrefix.get(prefix).add(namespaceURI);

		if (!prefixesByNamespaceURI.containsKey(namespaceURI)) prefixesByNamespaceURI.put(namespaceURI, new ArrayList<String>(2));
	...
	public String getNamespaceURI(final String prefix)
	{
		...
		if (prefix.equals(XMLConstants.DEFAULT_NS_PREFIX)) return defaultNamespaceURI;
		...
		if (namespaceURIsByPrefix.containsKey(prefix)) return namespaceURIsByPrefix.get(prefix).get(0);
