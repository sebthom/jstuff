...
public class GuestBookServiceInMemoryImpl implements GuestBookService
{
	...
	public int addEntry(final AddGuestBookEntryCommand entry) throws PermissionDeniedException
	{
		...
		if (entry.parentEntryId != null)
		{
			...
			if (parent == null) throw new UnknownEntityException(GuestBookEntry.class, entry.parentEntryId);
		}
			...
			final SortBy<String>... sortBy)
	{
		...
		return new PagedListWithSortBy<GuestBookEntry, String>(//
				GuestBookEntry.class, //
				slice(sortInPlace(toEntries(CollectionUtils.filter(store.values(),
						property(GuestBookEntryEntity.class, "createdBy", equalTo(createdBy))))), start - 1, max), //
				start, //
				store.size() //
		);
	}
	...
	public int getEntriesOfAuthorCount(final String createdBy)
	{
		...
		for (final GuestBookEntryEntity e : store.values())
			if (createdBy.equals(e.getCreatedBy())) count++;
		return count;
	}
	...
	{
		if (fromIndex >= elements.size()) return Collections.emptyList();

		int toIndex = fromIndex + maxElements;
		if (toIndex >= elements.size()) toIndex = elements.size();
			...
			final SortBy<String>... sortBy) throws PermissionDeniedException
	{
		...
		final GuestBookEntryEntity entity = store.get(entryId);
		if (entity == null) throw new UnknownEntityException(GuestBookEntry.class, entryId);
	...
	private void remove(final GuestBookEntryEntity entity)
	{
		...
		// remove from parent
		if (entity.getParent() != null) entity.getParent().getResponses().remove(entity);
	...
	public int removeEntriesOfAuthor(final String createdBy) throws PermissionDeniedException
	{
		...
		final List<GuestBookEntryEntity> entities = CollectionUtils.filter(store.values(),
				property(GuestBookEntryEntity.class, "createdBy", equalTo(createdBy)));
		for (final GuestBookEntryEntity e : entities)
			remove(e);
