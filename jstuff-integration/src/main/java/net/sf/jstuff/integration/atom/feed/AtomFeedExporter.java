...
public abstract class AtomFeedExporter extends RemoteExporter implements HttpRequestHandler
{
	...
	protected AtomEntry[] getEntries(final HttpServletRequest request)
	{
		...
		for (final SimpleEntry< ? > se : getSimpleEntries(request))
		{
			...
			entry.setId("tag:" + request.getServerName() + "," + DF.format(se.getDateCreated()) + ":" + feedId + "/"
					+ se.getId());
			entry.setTitle(se.getSubject());
			//entry.setLink(new AtomLink("TODO", AtomLink.REL_RELATED, null));
			entry.setSummary(new AtomText(StringUtils.htmlToPlainText(StringUtils.emptyIfNull(se.getContent()))
					.toString()));
			entry.setContent(new AtomText(se.getContent(), se.getContent().indexOf('<') > -1 ? AtomText.TYPE_HTML
					: AtomText.TYPE_TEXT));
	...
	{
		final AtomFeed atomFeed = new AtomFeed("tag:" + request.getServerName() + "," + DateUtils.getCurrentYear()
				+ ":" + feedId);
		atomFeed.setLogo(getLogoURL(request));
		atomFeed.setTitle(feedTitle);
		atomFeed.setSubtitle(feedSubTitle);
		atomFeed.setLinks(new AtomLink(request.getRequestURL().toString(), AtomLink.REL_SELF,
				AtomLink.TYPE_APPLICATION_ATOM_XML));
