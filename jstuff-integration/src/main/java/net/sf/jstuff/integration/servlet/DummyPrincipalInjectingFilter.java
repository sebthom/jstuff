...
public class DummyPrincipalInjectingFilter implements Filter
{
	...
	public void init(final FilterConfig config) throws ServletException
	{
		// configure based on system properties
		{
			...
			if (userRoles != null) setUserRoles(userRoles.split(USER_ROLES_SEPARATOR));
		}
		...
		if (user.getName() == null && DEFAULT_USER_NAME != null) setUsername(DEFAULT_USER_NAME);
		if (userRoles.size() == 0 && DEFAULT_USER_ROLES != null) setUserRoles(DEFAULT_USER_ROLES);
	...
	public void setUserRoles(final Collection<String> userRoles)
	{
		...
		if (userRoles != null) this.userRoles.addAll(userRoles);
	}

	public void setUserRoles(final String... userRoles)
	{
		this.userRoles.clear();
		if (userRoles != null) CollectionUtils.addAll(this.userRoles, userRoles);
	}
}
